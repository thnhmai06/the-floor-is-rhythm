...
namespace Work::Render::Layers
{
	...
	inline std::unique_ptr<Layer> fail;
	inline std::unique_ptr<Layer> pass;
	...
	inline void init_all()
	{
		...
		fail = std::make_unique<Layer>();
		pass = std::make_unique<Layer>();
	...
	inline void reset_all(const bool to_initial_state = false)
	{
		...
		if (fail) fail->reset(to_initial_state);
		if (pass) pass->reset(to_initial_state);
		if (foreground) foreground->reset(to_initial_state);
