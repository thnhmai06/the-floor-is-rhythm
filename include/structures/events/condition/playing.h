...
namespace Structures::Events::Condition
{
	namespace Playing
	{
		struct HitSound : Condition
		{
			...
			{
				static constexpr auto ALL = Game::Beatmap::Hitsound::Addition::Normal;

				Game::Beatmap::Hitsound::Addition condition = ALL;

				[[nodiscard]] bool is_satisfied(const Game::Beatmap::Hitsound::Additions& hit_sound) const;
			};
			struct HitSampleCondition
			{
				static constexpr auto ALL_CONDITION = Game::Beatmap::Hitsound::SampleSet::NoCustom;

				Game::Beatmap::Hitsound::SampleSet normal = ALL_CONDITION;
				Game::Beatmap::Hitsound::SampleSet addition = ALL_CONDITION;
