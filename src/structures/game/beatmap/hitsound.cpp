...
namespace Structures::Game::Beatmap::Hitsound
{
	...
	HitSample::HitSample(const std::string& hitsample_str)
	{
		if (hitsample_str.empty())
			return; // not written
		const auto list = Utilities::String::split(hitsample_str, Format::FileFormat::Beatmap::HitObjects::HitSample::DELIMITER);
	...
	std::string HitSample::to_string() const
	{
		...
		result.push_back(Format::FileFormat::Beatmap::HitObjects::HitSample::DELIMITER);
	...
	std::string HitSample::get_hitsound_filename(const HitsoundBitmap& HitsoundType)
	{
		...
		case SampleSetType::NO_CUSTOM:
			return ""; // No custom sampleSet, use skin hitsound instead
		}
