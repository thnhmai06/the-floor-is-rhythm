...
{
	if (!SDL_Init(SDL_INIT_VIDEO)) throw SDL_Exception::SDL::SDL_InitSDL_Failed();

	window = SDL_CreateWindow(NAME, 1366, 768, SDL_WINDOW_HIDDEN);
	if (!window) throw SDL_Exception::SDL::SDL_CreateWindow_Failed();
	...
	if (!device) throw SDL_Exception::SDL::SDL_CreateGPUDevice_Failed();

	if (!SDL_ClaimWindowForGPUDevice(device, window)) throw SDL_Exception::SDL::SDL_ClaimWindowForGPUDevice_Failed();
